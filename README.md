# Hi 👋, I'm Seif Mattar

### Full-Stack Web Developer | React • Next.js • Node.js

I’m a **Full-Stack Web Developer** and **Business Information Systems (BIS) student at AAST**, focused on building responsive, scalable, and high-performance web applications.

I enjoy working across the **frontend, backend, databases, and cloud**, turning ideas into practical digital products with clean architecture and a strong user experience.

---

<div align="center">

[![GitHub followers](https://img.shields.io/github/followers/YOUR_USERNAME?style=for-the-badge\&logo=github\&label=Followers)](https://github.com/YOUR_USERNAME)
[![GitHub stars](https://img.shields.io/github/stars/YOUR_USERNAME?style=for-the-badge\&logo=github\&label=Stars)](https://github.com/YOUR_USERNAME)

</div>

---

## 🚀 About Me

* 🎓 Business Information Systems (BIS) student at **AAST**
* 💻 Focused on **Full-Stack Web Development**
* ⚛️ Building modern applications with **React & Next.js**
* 🛠️ Developing backend systems with **Node.js & Express.js**
* 🗄️ Working with **MySQL & MongoDB**
* ☁️ Working with **AWS services, including S3**
* 🔐 Interested in authentication, authorization, and secure APIs
* 📊 Interested in **Data Analysis & Business Intelligence**
* 🎨 Passionate about clean UI, responsive design, and great UX
* ⚡ Interested in performance, scalability, and maintainable code

---

## 🧑‍💻 Tech Stack

### Frontend

<p align="left">

<a href="https://react.dev/">
<img src="https://skillicons.dev/icons?i=react" width="50" height="50" alt="React" />
</a>

<a href="https://nextjs.org/">
<img src="https://skillicons.dev/icons?i=nextjs" width="50" height="50" alt="Next.js" />
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript">
<img src="https://skillicons.dev/icons?i=javascript" width="50" height="50" alt="JavaScript" />
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/HTML">
<img src="https://skillicons.dev/icons?i=html" width="50" height="50" alt="HTML5" />
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/CSS">
<img src="https://skillicons.dev/icons?i=css" width="50" height="50" alt="CSS3" />
</a>

</p>

### UI & Styling

<p align="left">

<a href="https://tailwindcss.com/">
<img src="https://skillicons.dev/icons?i=tailwind" width="50" height="50" alt="Tailwind CSS" />
</a>

<a href="https://getbootstrap.com/">
<img src="https://skillicons.dev/icons?i=bootstrap" width="50" height="50" alt="Bootstrap" />
</a>

<a href="https://mui.com/">
<img src="https://skillicons.dev/icons?i=materialui" width="50" height="50" alt="Material UI" />
</a>

<a href="https://www.framer.com/motion/">
<img src="https://skillicons.dev/icons?i=framer" width="50" height="50" alt="Framer Motion" />
</a>

</p>

### Backend

<p align="left">

<a href="https://nodejs.org/">
<img src="https://skillicons.dev/icons?i=nodejs" width="50" height="50" alt="Node.js" />
</a>

<a href="https://expressjs.com/">
<img src="https://skillicons.dev/icons?i=express" width="50" height="50" alt="Express.js" />
</a>

<a href="https://www.postman.com/">
<img src="https://skillicons.dev/icons?i=postman" width="50" height="50" alt="Postman" />
</a>

</p>

### Databases

<p align="left">

<a href="https://www.mysql.com/">
<img src="https://skillicons.dev/icons?i=mysql" width="50" height="50" alt="MySQL" />
</a>

<a href="https://www.mongodb.com/">
<img src="https://skillicons.dev/icons?i=mongodb" width="50" height="50" alt="MongoDB" />
</a>

</p>

### Cloud & Tools

<p align="left">

<a href="https://aws.amazon.com/">
<img src="https://skillicons.dev/icons?i=aws" width="50" height="50" alt="AWS" />
</a>

<a href="https://git-scm.com/">
<img src="https://skillicons.dev/icons?i=git" width="50" height="50" alt="Git" />
</a>

<a href="https://github.com/">
<img src="https://skillicons.dev/icons?i=github" width="50" height="50" alt="GitHub" />
</a>

<a href="https://code.visualstudio.com/">
<img src="https://skillicons.dev/icons?i=vscode" width="50" height="50" alt="VS Code" />
</a>

</p>

### Data & Business Intelligence

<p align="left">

<a href="https://powerbi.microsoft.com/">
<img src="https://skillicons.dev/icons?i=powerbi" width="50" height="50" alt="Power BI" />
</a>

</p>

---

## ⭐ Featured Projects

### 🤝 AWN — Volunteer Platform

**AWN** is a full-stack volunteer platform designed to connect volunteers with organizations and create a professional environment for discovering opportunities, managing applications, and interacting with organizations.

**Tech Stack**

`Next.js` `React` `Node.js` `Express.js` `MySQL` `REST API` `JWT` `AWS`

**Key Features**

* 🔐 Authentication & authorization
* 👤 User and organization workflows
* 📋 Volunteer opportunity management
* 📝 Application management
* 📊 Dashboard-based experiences
* 🔌 RESTful API architecture
* 🗄️ Relational database design
* 📱 Responsive user interface
* ☁️ Cloud-based file handling

**What this project demonstrates**

> Full-stack development, frontend architecture, backend API development, database integration, authentication, responsive UI, and cloud integration.

**Repository:**
[View AWN on GitHub](YOUR_AWN_REPOSITORY_URL)

---

### 🌐 Full-Stack Web Applications

I build modern web applications using a combination of:

* **Next.js & React** — frontend architecture and UI
* **Node.js & Express.js** — backend services and APIs
* **MySQL & MongoDB** — data persistence
* **REST APIs** — frontend/backend communication
* **JWT** — authentication and authorization
* **AWS S3** — cloud-based file storage
* **Tailwind CSS / Bootstrap / Material UI** — responsive interfaces

---

## 🧠 Development Interests

```text
Full-Stack Web Development
RESTful API Design
Database Architecture
Authentication & Authorization
Cloud Services
Web Performance
Responsive UI/UX
Scalable Application Architecture
Data Analysis
Business Intelligence
```

---

## 📊 GitHub Statistics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github" alt="Seif's GitHub Stats" />

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight&hide_border=true" alt="Top Languages" />

</div>

---

## 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=YOUR_USERNAME&theme=tokyonight&hide_border=true" alt="GitHub Streak" />

</div>

---

## 📈 Contribution Graph

<div align="center">

[![Seif's GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=YOUR_USERNAME\&theme=tokyo-night\&hide_border=true)](https://github.com/YOUR_USERNAME)

</div>

---

## 🎯 2026 Goals

* 🚀 Build and deploy more production-ready full-stack applications
* ⚛️ Deepen my knowledge of **React & Next.js**
* 🏗️ Improve backend architecture and API design
* ☁️ Expand my knowledge of **AWS and cloud architecture**
* 🗄️ Improve database design and optimization
* 🔐 Learn more about application security
* 📊 Combine software development with **data & business intelligence**
* 🤝 Contribute to open-source projects
* 📚 Continuously improve my software engineering practices

---

## 📚 Currently Learning

* Advanced **Next.js**
* Scalable backend architecture
* API security and authentication
* Cloud deployment and AWS
* Database optimization
* Software architecture
* Performance optimization
* CI/CD and developer tooling

---

## 🤝 Let's Connect

<div align="left">

<a href="https://www.linkedin.com/in/YOUR_LINKEDIN_USERNAME">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

<a href="mailto:YOUR_EMAIL">
<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>

<a href="YOUR_PORTFOLIO_URL">
<img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
</a>

</div>

---

<div align="center">

### 💡 Building ideas into scalable digital solutions.

⭐ **If you find my projects interesting, feel free to explore my repositories!**

</div>
